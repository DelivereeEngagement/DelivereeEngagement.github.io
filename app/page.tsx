...
export default function Home(){
  ...
  return <main className="shell"><section className="card"><div className="brand">deliver<span>ee</span></div>
    ...
    {screen==="result"&&<Status icon="✓" title="บันทึกข้อมูลเรียบร้อยแล้ว" english="Information submitted" detail={`ชื่อ (Name): ${result.name}\nเบอร์โทรศัพท์ (Phone): ${result.phone}\nรหัสอ้างอิง (Ref): ${result.reference}\n\n${result.confirmationSent?"ส่งข้อความยืนยันไปยังแชท LINE แล้ว":"ไม่สามารถส่งข้อความอัตโนมัติได้ กรุณากดปุ่มด้านล่างเพื่อส่งข้อความยืนยัน"}`} action="กลับไปที่แชท LINE" onAction={()=>{if(result.confirmationSent&&window.liff?.isInClient()){window.liff.closeWindow()}else{location.href=OA_URL+encodeURIComponent(confirmation)}}}/>} 
